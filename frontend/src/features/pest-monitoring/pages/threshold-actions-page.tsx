...
export function ThresholdActions() {
  ...
  // Use filters from store (initialized from backend data)
  const storeFilters = useDashboardStore((state) => state.filters);
  const setStoreFilters = useDashboardStore((state) => state.setFilters);
  const {
  ...
  // Local filter state for this page (can override store filters)
  const [localFilters, setLocalFilters] = useState<FilterValues | null>(null);

  // Use local filters if set, otherwise use store filters
