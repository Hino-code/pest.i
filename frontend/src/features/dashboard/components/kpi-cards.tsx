  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <KpiCard
        ...
        value={kpis.averagePestCount.toFixed(1)}
        icon={GraphUp}
...
function KpiCard({
  ...
  const TrendIcon =
    trend.value > 0 ? GraphUp : trend.value < 0 ? GraphDown : Minus;
    ...
    <Card className="relative overflow-hidden border border-border/60 bg-gradient-to-br from-card to-muted/10 shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300 group rounded-xl">
      <div className="flex flex-col h-full p-6 space-y-5">
        ...
        <div className="flex items-center gap-2 justify-between">
          {/* Insight Subtext */}
          <span
            className={`text-xs font-medium leading-tight truncate flex-1 ${statusColor}`}
          >
            {label}
          </span>
          ...
          <div
            className={`flex items-center text-xs font-semibold ${statusColor}`}
          >
