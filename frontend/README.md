# Frontend - Early Warning System

A comprehensive React-based web application for monitoring pest activity in rice fields and forecasting future outbreaks using data-driven models.
...
This is the frontend application for the Early Warning System for Pest Monitoring and Forecasting. It provides real-time visualization, pest monitoring, forecasting, and alert generation to support data-driven agricultural decision-making.
...
| `VITE_USE_MOCKS`    | Enable mock data mode | `false`                 | No               |
...
1. **Express Auth Server** (Port 8000)
...
The Vite dev server is configured with proxies in `vite.config.ts` to route requests to the appropriate backend service.
...
- **Solution**: Ensure backend CORS configuration allows requests from your frontend URL
...
- **API.md** - API integration guide and endpoint documentation
